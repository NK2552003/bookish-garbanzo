...
export default function ContactForm() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      emailjs.init("jm_Y0wAADmEd2gYlo") // your public key
    }
  }, [])
  ...
  const sendEmail = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const formEl = event.currentTarget // store reference before await
    ...
    try {
      ...
      formEl.reset() // now works without error
    } catch (error) {
  ...
  const buttonVariants = {
    ...
    visible: { scale: 1, opacity: 1, transition: { duration: 0.5, ease: "easeOut", delay: 0.6 } }
  }
          ...
          <form onSubmit={sendEmail} className="space-y-5">
            ...
            <div className="pt-2 flex items-center justify-center text-center">
              <motion.button
                ...
                className="relative inline-block p-px font-semibold leading-6 text-white bg-white/20 shadow-2xl cursor-pointer rounded-2xl shadow-zinc-900 transition-transform duration-300 ease-in-out hover:scale-105 active:scale-95 text-xs sm:text-base group"
              >
